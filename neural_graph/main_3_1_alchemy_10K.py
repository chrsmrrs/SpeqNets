...
class Alchemy_1(InMemoryDataset):
    ...
    def process(self):
        ...
        node_labels = pre.get_all_node_labels_allchem_3_1(True, True, indices_train, indices_val, indices_test)

        matrices = pre.get_all_matrices_3_1("alchemy_full", indices_train)
        matrices.extend(pre.get_all_matrices_3_1("alchemy_full", indices_val))
        matrices.extend(pre.get_all_matrices_3_1("alchemy_full", indices_test))
...
class NetGIN(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        x_new = torch.zeros(x.size(0), 646).to(device)

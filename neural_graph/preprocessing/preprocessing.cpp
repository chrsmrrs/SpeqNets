...
pair<vector<vector<uint>>, vector<vector<uint>>, vector<vector<uint>>> generate_local_sparse_am_3_2(const Graph &g) {
    ...
    for (Node i = 0; i < num_two_tuples; ++i) {
        ...
        Nodes w_neighbors = g.get_neighbours(w);
        for (Node w_n: w_neighbors) {
            unordered_map<ThreeTuple, Node>::const_iterator t = node_to_three_tuple.find(make_tuple(v, w_n, u));
        ...
        for (Node u_n: u_neighbors) {
            unordered_map<ThreeTuple, Node>::const_iterator t = node_to_three_tuple.find(make_tuple(v, w, u_n));

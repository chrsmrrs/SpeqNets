...
class aspirin_gnn(InMemoryDataset):
    def __init__(self, root, transform=None, pre_transform=None,
                 pre_filter=None):
        super(aspirin_gnn, self).__init__(root, transform, pre_transform, pre_filter)
        self.data, self.slices = torch.load(self.processed_paths[0])

    @property
    def raw_file_names(self):
        return "aspirin_1_1_FULL"
    ...
    def process(self):
        ...
        node_labels = pre.get_all_node_labels_1("aspirin", True)

        matrices = pre.get_all_matrices_1("aspirin", list(range(111763)))
...
dataset = aspirin_gnn(path, transform=MyTransform())
